...
Database::Database(const string& db, const string& un, const string& pw, time_t la)
    : db(db), username(un), password(pw), last_activity(la) {}
...
Database* Database::getInstance(const string& new_db, const string& new_username, const string& new_password, time_t last_activity)
{
    // Calling for the first time: this will set username and password
    if (instance == nullptr) 
    {
        instance = new Database(new_db, new_username, new_password, last_activity);
    }
...
Database& Database::operator=(const Database&&) noexcept(false)
{
...
bool Database::isTimeout()
{
    ...
    // -> returns true
    if (time(nullptr) - last_activity > TIMEOUT) return true;
